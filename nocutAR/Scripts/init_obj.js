function init_objects() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var GMP = $(xmlDoc).find("GMP");

            var select_type = $(xmlDoc).find("select_type");
            var select_type_content = $(xmlDoc).find("select_type_content");
            ...
            $(audios).each(function () {
                ...
                var type = $(this).find("type").text();
                var run_opt = $(this).find("run_opt").text();
                var btnkind = $(this).find("btnkind").text();   // 표시안함, 기본Button, 커스텀Button                
                var color = $(this).find("color").text();       // 기본Button Color
                if (color == '#0') {
                    color = "black";
                }
                var posX = $(this).find("posX").text();
            ...
            if(GMP && GMP[0] != "")
            {
                var lat = encodeURI(GMP.find("lat").text());
                var lng = encodeURI(GMP.find("lng").text());
                document.getElementById("viewGP").value = lat + ', ' + lng;
            }

            if (select_type && select_type[0] != "")
            {
                if (select_type[0].innerText == '1') {
                    ...
                    if (select_type_content)
                    {
                        var content = select_type_content[0].innerText;
                        document.getElementById("inspireTxt").value = content;
                    }
                }
                else if (select_type[0].innerText == '2') {
                    document.getElementById("show_Type_content").innerHTML = "<table cellpadding='0' cellspacing='0' align='center' width='485' style='padding-left:80px'><tbody><tr>"
                    + "<td width='366' height='250'>"
                    ...
                    if (select_type_content) {
                        for (var item = 0; item < select_type_content.length; item++) {
                            var content = select_type_content[item].innerText;
                            document.getElementById("addLoyaltyPlan").innerHTML += "Plan <input type='text' value=" + content + " name='loyalPlanTxt' style='width:300px;height:25px;' /><br />";
                        }
                    }                   
                }
                else if (select_type[0].innerText == '3') {
                    document.getElementById("show_Type_content").innerHTML = "<table cellpadding='0' cellspacing='0' align='center' width='485' style='padding-left:80px'><tbody><tr>"
                    + "<td width='366' height='250'>"
                    ...
                    if (select_type_content) {
                        var content = select_type_content[0].innerText;
                        document.getElementById("advertisementTxt").value = content;
                    }
                }
            }
...
function init_mark() {
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            HideProgress();
            alert("An error occurs while reading marker image.");
        }
    });
}